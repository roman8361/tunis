...
export default function TournamentClassicPage() {
  ...
  function getPairName(key: "A" | "B" | "C"): string {
    const pairIds = currentRound?.pairs[key] ?? [];
      ...
      <header className="bg-white/95 backdrop-blur shadow-sm sticky top-0 z-10 border-b border-sky-100">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-sm font-medium text-slate-500">{completedRounds}/{totalRounds} туров</div>
          </div>
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-4 py-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-52 shrink-0">
          ...
          <div className="mt-6">
            ...
            <div className="bg-white/80 backdrop-blur rounded-xl overflow-hidden text-xs">
              <div className="grid grid-cols-4 gap-0 px-2 py-1.5 bg-white/50 text-slate-400 font-semibold uppercase text-[10px]">
              ...
              {isRotating
                ? sortedPlayers.map((p, idx) => (
                    <div key={p.id} className={`grid grid-cols-4 gap-0 px-2 py-1.5 border-t border-white/40 ${idx === 0 && p.gamesPlayed > 0 ? "bg-yellow-50" : ""}`}>
                      <div className="col-span-2 font-medium text-slate-700 truncate">{p.name}</div>
                      <div className="text-center text-slate-600">{p.wins}</div>
                      <div className={`text-center font-medium ${p.pointsDiff > 0 ? "text-green-600" : p.pointsDiff < 0 ? "text-red-500" : "text-slate-400"}`}>
                        {p.pointsDiff > 0 ? `+${p.pointsDiff}` : p.pointsDiff}
                      </div>
                    </div>
                  ))
                : pairs.map((pair, idx) => (
                    <div key={pair.label} className={`px-2 py-1.5 border-t border-white/40 ${idx === 0 && pair.gamesPlayed > 0 ? "bg-yellow-50" : ""}`}>
                      <div className="grid grid-cols-4 gap-0">
                        <div className="col-span-2 font-medium text-slate-700 truncate text-[10px]">
                          {pair.players.map((p) => p.name).join(" / ")}
                        </div>
                        <div className="text-center text-slate-600">{pair.wins}</div>
                        <div className={`text-center font-medium ${pair.pointsDiff > 0 ? "text-green-600" : pair.pointsDiff < 0 ? "text-red-500" : "text-slate-400"}`}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground mt-0.5">
                      <span className="font-medium text-slate-600">A:</span> {getPairName("A")} &nbsp;·&nbsp;
                      <span className="font-medium text-slate-600">B:</span> {getPairName("B")} &nbsp;·&nbsp;
                      <span className="font-medium text-slate-600">C:</span> {getPairName("C")}
                    </div>
                  </div>
                ...
                {currentGame && (
                  <div className="pb-6">
                    ...
                    <div className="text-xs text-slate-500 mb-4 bg-slate-50 rounded-xl px-3 py-2">
                      ...
                      {getPairName(currentGame.judgeKey)}
                    </div>
                    ...
                    <div className="flex items-stretch gap-4 mb-5">
                      <div className={`flex-1 rounded-xl p-4 border-2 transition-all ${currentGame.completed && currentGame.winner === "A" ? "border-green-400 bg-green-50" : "border-border bg-background"}`}>
                        ...
                        <div className="font-semibold text-foreground">{getPairName(currentGame.pairAKey)}</div>
                      ...
                      <div className={`flex-1 rounded-xl p-4 border-2 transition-all ${currentGame.completed && currentGame.winner === "B" ? "border-green-400 bg-green-50" : "border-border bg-background"}`}>
                        ...
                        <div className="font-semibold text-foreground">{getPairName(currentGame.pairBKey)}</div>
                        {currentGame.completed && (
